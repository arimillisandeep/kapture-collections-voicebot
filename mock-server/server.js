...
const handlers = {
  get_account_details(args, call) {
    call.state = call.state === STATES.INIT ? STATES.AUTH_PENDING : call.state;
    ...
    if (call.state === STATES.AUTHENTICATED) call.state = STATES.NEGOTIATION;
  ...
  verify_customer(args, call) {
    ...
    call.state = STATES.AUTH_PENDING;
    ...
    call.verified = true;
    call.state = STATES.AUTHENTICATED; // Only successful verification can set this state.
  ...
  log_promise_to_pay(args, call) {
    ...
    call.ptps.push(ptp); call.state = STATES.PTP_COLLECTED;
  ...
  mark_disposition(args, call) {
    ...
    call.dispositions.push(entry); call.state = STATES.CALL_ENDED;
...
app.post('/webhook', (req, res) => {
  ...
  const call = getCall(callIdFrom(req.body));
  const results = toolCalls.map((toolCall) => {
    ...
    log('tool_called', { name, callState: call.state, ok: payload.ok });
